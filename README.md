...
Skill 会根据同一编号、材料类型文件夹和最近的 `（拟定）` 文件，自动归档为：

```text
年度工作总结 / 我最终修改稿 / 002-季度工作总结-我最终修改稿.docx
```

如果同一个文件夹里有多份未归档的拟定稿，Skill 会先让你选择。

后续想让 Skill 学习你的修改习惯，只需要说：

```text
请帮我更新写作风格，根据刚生成的“002-季度工作总结”的几份稿件。
```

Skill 会自动找同一编号下的 `AI原始初稿`、`AI规范修订稿` 和 `我最终修改稿`。只有找不到或出现多组候选时，才会让你补充或选择。

风格回流时，Skill 主要对比 `AI规范修订稿` 和 `我最终修改稿`，这样学到的是你的修改习惯；`AI原始初稿` 和 `AI规范修订稿` 的差异，主要用来看公文规范子 Skill 修了什么。
